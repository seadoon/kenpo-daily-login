...
**毎月10回ログインすると100ポイント**がもらえる特典があるため、それを自動で達成することが目的です。平日（月〜金）に実行することで月10回の条件を確実にクリアします。
...
以上で完了です。次の平日 8:45 JST から自動実行されます。
...
**平日（月〜金）8:45 JST** に自動実行されます。月約20回実行されるので、10回の条件を余裕でクリアします。

変更したい場合は `.github/workflows/daily-kenpo-login.yml` の `cron` を編集してください。

```yaml
- cron: '45 23 * * 0-4'  # 日〜木 23:45 UTC = 月〜金 08:45 JST
```
